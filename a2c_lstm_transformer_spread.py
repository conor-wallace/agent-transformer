...
from opponent_transformer.models.pretrained_spread_opponents import get_opponent_actions
...
class Args:
    ...
    wandb_project_name: str = "simple_spread"
...
class Agent(nn.Module):
    def __init__(self, envs, embedding_dim):
        ...
        self.act_dim = envs.action_space[2].n
        self.obs_dim = envs.observation_space[2].shape[0]
        self.opp_act_dim = sum([envs.action_space[i].n for i in range(2)])
        self.opp_obs_dim = sum([envs.observation_space[i].shape[0] for i in range(2)])
...
def evaluate(agent, opponent_model, args):
    ...
    obs = eval_envs.reset()
    agent_obs = torch.FloatTensor([o[2] for o in obs]).to(device)
    modelled_agent_obs = [o[:2] for o in obs]
    ...
    obs_seq[:, -1] = agent_obs
    last_actions_seq = torch.ones((args.num_eval_envs, args.seq_length, agent.act_dim), device=device, dtype=torch.float32)
    reward_seq = torch.zeros((args.num_eval_envs, args.seq_length, 1), device=device, dtype=torch.float32)
    # rtg_seq[:, -1] = torch.tensor([args.target_return] * args.num_eval_envs, device=device, dtype=torch.float32).reshape(args.num_eval_envs, 1)
    ...
    for t in range(args.episode_length):
        ...
        next_agent_obs = torch.FloatTensor([o[2] for o in next_obs])
        next_modelled_agent_obs = [o[:2] for o in next_obs]
        rewards = torch.FloatTensor([r[2] for r in rewards]).unsqueeze(1)
...
if __name__ == "__main__":
    ...
    opponent_model = BERTRAOOpponentTransformer(
        ...
        oppnt_state_dims=[envs.observation_space[i].shape[0] for i in range(2)],
        oppnt_act_dims=[envs.action_space[i].n for i in range(2)],
        hidden_dim=128,
    ...
    last_actions_seqs = torch.zeros((args.num_minibatches, args.batch_size, args.num_envs, args.seq_length, agent.act_dim)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        for idx in range(args.num_minibatches):
            if counter == 0:
                ...
                # policy data
                all_obs = envs.reset()
                next_obs = torch.tensor([o[2] for o in all_obs]).to(device, dtype=torch.float32)
                next_opp_obs = [o[:2] for o in all_obs]
                ...
                obs_seq[:, -1] = next_obs
                last_actions_seq = torch.zeros((args.num_envs, args.seq_length, agent.act_dim), device=device, dtype=torch.float32)
            ...
            for step in range(0, args.batch_size):
                ...
                next_obs = torch.tensor([o[2] for o in all_next_obs]).to(device, dtype=torch.float32)
                next_opp_obs = [o[:2] for o in all_next_obs]
                next_rewards = torch.tensor([r[2] for r in all_rewards]).unsqueeze(-1).to(device, dtype=torch.float32)
